...
export const columns: (onUpdate: (billId: number, newStatus: "Pending" | "Successful") => void) => ColumnDef<Bill>[] = (onUpdate) =>[
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className="font-medium ml-6">{formatted}</div>
    },
  },
  {
    accessorKey: "status",
    header: "Status",
    cell: ({ row }) => {
      return row.getValue("status")==="Pending"? <Label className="text-red-500 ml-[-10px]">Pending</Label> : <Label className="text-green-500 ml-[-10px]">Successful</Label>
    }
  },
  {
    id: "actions",
